...
import com.nowbook.user.dto.BuyerDto;
import com.nowbook.user.endpoint.NbUserClient;
import com.nowbook.user.manager.AccountManager;
...
import com.nowbook.user.mysql.UserProfileDao;
import com.nowbook.user.util.RSAToBCDCoder;
...
public class AccountServiceImpl implements AccountService<User> {
    ...
    @Autowired
    private NbUserClient nbUserClient;
    ...
    public Response<User> userLogin(String id, LoginType type, String password) {
        ...
        try {
            ...
            if (isEmpty(storedPassword)) {   // 密码为空则需要校验锦霖旧有系统的用户权限
    ...
    public Response<User> userLoginSms(String id, LoginType type) {
        Response<User> result = new Response<User>();
    ...
    public Response<User> WxLogin(String id, LoginType type, String password) {
        Response<User> result = new Response<User>();
        ...
        try {
            ...
            if (isEmpty(storedPassword)) {   // 密码为空则需要校验锦霖旧有系统的用户权限
                ...
                checkState(wxPasswordMatch(password, storedPassword), "user.password.incorrect");
            }
            result.setResult(user);
    ...
    private Response<Boolean> checkUserTokenOfElder(String id, String password) {
        return nbUserClient.check(id, password);
    }
    ...
    private boolean wxPasswordMatch(String password, String encryptedPassword) {
        return Objects.equal(password, encryptedPassword);
    }
     ...
     */
    @Override
    public Response<Boolean> changePassword(Long userId, String oldPassword, String newPassword,String type) {
        Response<Boolean> result = new Response<Boolean>();
        if (userId == null) {
            log.error("id can not be null");
            result.setError("id.not.null.fail");
        ...
        userExtraUpdate.setUserId(userId);
        try {
            if(type.equals("1")){
                if(newPassword.length() <6 || newPassword.length() >16){
                    log.error("new password need 6~16 length", newPassword);
                    result.setError("new password need 6~16 length");
                    return result;
                }
                ...
                if(newPassword.length() !=6 || !isNumeric(newPassword)){
                    log.error("new password need six number", newPassword);
                    result.setError("new password need six number");
                    return result;
                }
                userExtraUpdate.setPayPassword(encryptPassword(newPassword));
                User user = userDao.findById(userId);
                if (user == null) {
                    log.error("can not find user whose id={}", userId);
                    result.setError("user.not.found");
                ...
                if (userExtra == null) {
                    userExtraDao.create(userExtraUpdate);
                }else{
                    userExtraDao.update(userExtraUpdate);
                }
                ...
                if (userExtra == null) {
                    log.error("can not find user whose id={}", userId);
                    result.setError("userExtra.not.found");
                    return result;
                }
                if (!passwordMatch(oldPassword, userExtra.getPayPassword())) {
                    log.error("payPassword not match for user(id={})", userId);
                    result.setError("userExtra.payPassword.incorrect");
                    return result;
                }
                userExtraDao.update(userExtraUpdate);
            }
//            userCache.invalidate(userId);
     ...
     */
    @Override
    public Response<Boolean> resetPassword(Long userId, String password,String type) {
        Response<Boolean> result = new Response<Boolean>();
        if (userId == null) {
        ...
        try {
            if(type.equals("1")){
                ...
                UserExtra updated = new UserExtra();
                updated.setUserId(userExtra.getUserId());
                updated.setPayPassword(encryptPassword(password));
                userExtraDao.update(updated);
            }
            userCache.invalidate(userId);
            result.setResult(Boolean.TRUE);
            return result;
    ...
    public Response<Long> findByOpenId(String openId){
        Response<Long> result = new Response<Long>();
//        Long userId = userDao.findByOpenId(openId);
