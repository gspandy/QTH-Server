package com.nowbook.image.yupoo;
...
@Slf4j
public class UpYun implements ImageServer {

    private final DateTimeFormatter dateTimeFormatter = DateTimeFormat.forPattern("/yyyy/M/dd/");
    ...
    public String write(String fileName, MultipartFile file) throws ImageUploadException {
        try {
            ...
            boolean isSucceed = writeFile(fileName,imageData,true);
            if(!isSucceed){
                throw new ImageUploadException("Write failed");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String handleFileName(String originalName, byte[] imageData) {
        ...
        String newName = hf.newHasher().putBytes(imageData).hash().toString() + "." + ext;
        return dateTimeFormatter.print(DateTime.now()) + newName;
    }
