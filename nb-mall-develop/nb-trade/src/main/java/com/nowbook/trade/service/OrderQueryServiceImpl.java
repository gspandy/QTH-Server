...
public class OrderQueryServiceImpl implements OrderQueryService {
    ...
    private static final List<NbOrder> EMPTY_HAIER_LIST = Collections.emptyList();
     ...
     * @param pageNo             页码
     * @param size               返回条数
     * @param finishedStartAt     完成起始时间
     * @param finishedEndAt       完成截止时间
     * @return 订单列表
     */
    @Override
    public Response<Paging<Order>> findByCreateSettlement(Order criteria, Integer pageNo, Integer size, Date finishedStartAt, Date finishedEndAt) {
        Response<Paging<Order>> result = new Response<Paging<Order>>();
        try {
            PageInfo pageInfo = new PageInfo(pageNo, size);
            ...
            params.put("limit", pageInfo.limit);
            params.put("criteria", criteria);
            params.put("finishedStartAt", finishedStartAt);
            params.put("finishedEndAt", finishedEndAt);
            Paging<Order> orders = orderDao.findBy(params);
            result.setResult(orders);
            return result;
        } catch (Exception e) {
    ...
    public Response<RichOrderWithDetail> buyerFindOrderById(@ParamInfo("buyer") BaseUser buyer, @ParamInfo("id") Long id) {
        ...
        try {
            RichOrderBuyerView robv = makeRichOrderBuyerView(order, null);
    ...
    public Response<List<RichOrderItem>> findOrderItemsByOrderIdForComment(
        ...
        RichOrderBuyerView buyerView = makeRichOrderBuyerView(order, null);
    ...
    private RichOrderSellerView makeRichOrderSellerView(Order order, boolean isAdmin) {
        ...
        try {
            richOrderSellerView.setPayedAt(order.getPaidAt());
...
//                }
    ...
    public Response<Paging<RichOrderBuyerView>> findByBuyerId(BaseUser baseUser, Integer pageNo,
                                                              Integer size, Integer status, Long orderId, Long distributorId) {
        Response<Paging<RichOrderBuyerView>> result = new Response<Paging<RichOrderBuyerView>>();

        if(baseUser == null && distributorId == null){
        ...
        try {
            ...
            if(orderId != null) {
                ...
                if(order != null && Objects.equal(order.getBuyerId(), baseUser.getId())) {
                    ...
                    List<RichOrderBuyerView> richOrderBuyerViews = buildRichOrdersForBuyerView(Lists.newArrayList(order), distributorId);
            ...
            if (distributorId != null) {
                params.put("distributorId", distributorId.toString());
            }
            ...
            if (orderIds.getTotal() > 0) {
                List<Order> orders = orderDao.findByIds(orderIds.getData());
                List<RichOrderBuyerView> richOrderBuyerViews = buildRichOrdersForBuyerView(orders, distributorId);
     ...
     */
    @Override
    public Response<Paging<RichOrderBuyerView>> findByBuyerIdForNS(BaseUser baseUser, Integer pageNo,
                                                              Integer size, Integer status, Long orderId, Long distributorId) {
        Response<Paging<RichOrderBuyerView>> result = new Response<Paging<RichOrderBuyerView>>();
        ...
        try {
            ...
            Paging<Long> orderIds;

            if(status !=null && status.equals(-4)){
                orderIds = orderItemDao.findOrderIdsByForNs(params, pageInfo.getOffset(), pageInfo.getLimit());
            }else{
                orderIds = orderItemDao.findOrderIdsBy(params, pageInfo.getOffset(), pageInfo.getLimit());
            }
    ...
    private List<RichOrderBuyerView> buildRichOrdersForBuyerView(List<Order> orders, Long distributorId) {
        List<RichOrderBuyerView> result = Lists.newArrayListWithCapacity(orders.size());
        for (Order order : orders) {
            try {

                RichOrderBuyerView richOrderBuyerView = makeRichOrderBuyerView(order, distributorId);
                ...
                richOrderBuyerView.setUserTradeInfo(tradeInfo);
                richOrderBuyerView.setPaymentPlatform(order.getPaymentPlatform());
                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String dateString = formatter.format(richOrderBuyerView.getCreatedAt());
                richOrderBuyerView.setCreatedAtTime(dateString);
                richOrderBuyerView.setIsBalance(order.getIsBalance());
    ...
    private RichOrderBuyerView makeRichOrderBuyerView(Order order, Long distributorId) {
...
//        Item item = itemService.findById(richOrderItem.getSku().getItemId()).getResult();
            richOrderBuyerView.setPayedAt(order.getPaidAt());
            richOrderBuyerView.setNtalkerId(shopExtra.getNtalkerId());
            richOrderBuyerView.setSiteId(order.getSellerId());
            richOrderBuyerView.setShopId(shop.getId());
...
//                }
            ...
            if (distributorId != null && !"".equals(distributorId)) {
                //查询分销商信息
                DistributionInfoForQuery distributionInfo = distributionsDao.selectDistributionByKey(distributorId);
                richOrderBuyerView.setDistributionInfo(distributionInfo);
                //查询订单佣金
                AmountDetail amountDetail = new AmountDetail();
                amountDetail.setOrderId(order.getId().toString());
                amountDetail.setDistributorsId(distributorId);
                Paging<AmountDetail> AmountDetail = amountDetailDao.selectAmountDetail(amountDetail, 0, 1);
                if (AmountDetail.getData().size() == 1) {
                    richOrderBuyerView.setAmountDetail(AmountDetail.getData().get(0));
                }
            }
    ...
    private void buildRichOrder(Order order, RichOrder richOrder) {
        ...
        //是否是enb商家
        richOrder.setIsEnb(Objects.equal(eNbSellerId, order.getSellerId().toString()));
        ...
        for (OrderItem orderItem : orderItems) {
            ...
            level.setLevel(orderItem.getLevel());
            List<Level> levelList =levelDao.selectByLevel(level);
            if(levelList ==null || levelList.size() ==0){
                log.error("failed to find level(level={}),skip", orderItem.getLevel());
                continue;
            }

            //通过sku获取对应的各个等级价格 2017-09-27 dpzh
            sku.setSalePrice(levelPriceService.getUserLevelPrice(sku));
            if(sku.getPriceType()==2){
                sku.setPrice(sku.getSellingPrice());
            }if(sku.getPriceType()==3){
                sku.setPrice(sku.getCustomPrice());
            }
            ...
            roi.setPriceType(item.getPriceType());
        ...
        richOrder.setPaymentPlatform(order.getPaymentPlatform());
    }
    ...
    /**
     *
     * 获取指定日期内更新的订单列表，封装成Page<NbOrder>对象返回
     *
     ...
     * @return  更新的订单列表
     */
    public Response<Paging<NbOrder>> findNbOrderByUpdatedAt(Date beginAt,
                                                                  Date endAt,
                                                                  List<Long> businesses,
                                                                  Integer pageNo,
                                                                  Integer size) {
        Response<Paging<NbOrder>> result = new Response<Paging<NbOrder>>();
        ...
        try {
            ...
            if (len == 0) {
                Paging<NbOrder> nbOrderPaging = new Paging<NbOrder>(0L, EMPTY_HAIER_LIST);
                result.setResult(nbOrderPaging);
                return result;
            }
            ...
            List<NbOrder> nbOrders = getNbOrders(paging, orderItems, mappedUserInfo, mappedOrderExtra, mappedSku, mappedBuyer);

            Paging<NbOrder> nbOrderPaging = new Paging<NbOrder>(paging.getTotal(), nbOrders);
            result.setResult(nbOrderPaging);
            return result;
            ...
            log.error("fail to query nb orders page with beginAt={}, endAt={}, pageNo={}, size={}",
    ...
    /**
     *
     * 获取指定卖家ID并且在指定日期内更新的订单列表，封装成Page<NbOrder>对象返回
     *
     * @param beginAt       查询开始时间
     * @param endAt         查询截止时间
     * @param businesses    行业范围筛选
     ...
     * @return  更新的订单列表
     */
    public Response<Paging<NbOrder>> findNbOrderByUpdatedAtAndSellerIds(Date beginAt,
                                                                              Date endAt,
                                                                              List<Long> businesses,
                                                                              Integer pageNo,
                                                                              Integer size,
     ...
     * @return 封装后的锦霖订单，包含子订单信息及交易信息和发票信息
     */
    private List<NbOrder> getNbOrders(Paging<Order> orderPaging,
        ...
        List<NbOrder> nbOrders = Lists.newLinkedList();
        Map<Long, NbOrder> maps = Maps.newTreeMap();
        ...
        for (Order order : orderPaging.getData()) {
            try {
                ...
                // 填充商家8码
                NbOrder dto = NbOrder.transform(order, shopExtra.getShopId(), shopExtra.getOuterCode());
        ...
        for (OrderItem orderItem : items) {
            NbOrderItem itemDto = NbOrderItem.transform(orderItem);
            NbOrder orderDto = maps.get(orderItem.getOrderId());
            List<NbOrderItem> orderItems = orderDto.getItems();
     ...
     * 填入商品信息
     *
     * @param itemDto 锦霖子订单对象
     * @param sku     库存
     */
    private void fillWithSku(NbOrderItem itemDto, Sku sku) {
     ...
     */
    private void fillWithTradeInfo(NbOrder dto, UserTradeInfo tradeInfo) {
        // 电话号码头尾不允许空格
...
//            dto.setDistrictCode(tradeInfo.getCityCode());
     ...
     * @param extra 订单补充信息
     */
    private void fillWithOrderExtraAndTradeInfo(NbOrder dto, OrderExtra extra, UserTradeInfo userTradeInfo) {
    ...
    /**
     * 获取指定id的订单，封装成NbOrder返回
     *
     * @param id 订单号
     * @return 订单
     */
    public Response<NbOrder> findNbOrderById(Long id) {
        Response<NbOrder> result = new Response<NbOrder>();
        try {
            Order order = orderDao.findById(id);
            ...
            NbOrder nbOrder = NbOrder.transform(order, items, shopExtra.getShopId(), shopExtra.getOuterCode());
            OrderExtra extra = orderExtraDao.findByOrderId(order.getId());
            UserTradeInfo tradeInfo = userTradeInfoDao.findById(order.getTradeInfoId());

            if (tradeInfo != null)
                fillWithTradeInfo(nbOrder, tradeInfo);

            if (extra != null)
                fillWithOrderExtraAndTradeInfo(nbOrder, extra, tradeInfo);

            List<NbOrderItem> nbOrderItems = nbOrder.getItems();

            int itemLen = nbOrderItems.size();
            List<Long> skuIds = Lists.newArrayListWithCapacity(itemLen);

            for (NbOrderItem nbOrderItem : nbOrderItems) {
                skuIds.add(nbOrderItem.getSkuId());
            }

            Map<Long, Sku> mappedSku = getMappedSku(skuIds);

            for (NbOrderItem nbOrderItem : nbOrderItems) {
                Sku sku = mappedSku.get(nbOrderItem.getSkuId());
                if (sku != null) {
                    fillWithSku(nbOrderItem, sku);
                }
                log.info("----------"+nbOrderItem.getStatus());
                System.out.println("----------"+nbOrderItem.getStatus());
                if(nbOrderItem.getStatus().equals(OrderItem.Status.DONE.value())){
                    String return_day_limit = systemConfRedisDao.getDate("return_day_limit");
                    log.info("----------"+new Date().getTime());
                    log.info("----------"+order.getDoneAt().getTime());
                    log.info("----------"+systemConfRedisDao.getDate("return_day_limit"));
                    System.out.println("----------"+new Date().getTime());
                    System.out.println("----------"+order.getDoneAt().getTime());
                    System.out.println("----------"+systemConfRedisDao.getDate("return_day_limit"));
                    if(new Date().getTime() - order.getDoneAt().getTime()>Long.valueOf(return_day_limit)*24*60*60*1000){
                        nbOrderItem.setStatus(8);
                    };
                }
            }
            ...
            nbOrder.setBuyerName(buyer.getName());

            result.setResult(nbOrder);
            return result;
        } catch (IllegalStateException e) {
            log.error("fail to query nb order with id={}, error:{}", id, e.getMessage());
            result.setError("order.get.fail");
            return result;
        } catch (Exception e) {
            log.error("fail to query nb order with id={}", id, e);
            result.setError("order.get.fail");
            return result;
        }
    }
    ...
    private List<RichOrderItem> getRichOrderItem(Map<Long, Integer> skuIdAndQuantity) {
        ...
        for (Long skuId : skuIdAndQuantity.keySet()) {
            ...
            //是否是enb商家
            roi.setIsEnb(Objects.equal(eNbSellerId, item.getUserId().toString()));
...
//            }
     ...
     * 订单状态status统计各个状态的数量
     * @author dpzh
     * @create 2017/7/24 16:23
     * @param id  买家id
     * @return  各种状态对应的数量 订单状态 0：待付款(0)，1：待发货(1)，2：待收货(2)，3：退换货（4，5，6,-3,-4），4：已完成（3）
     */
    public Map<String,String> countStatus(Long id){
        List<Order> list=orderDao.countStatus(id);
        Integer obligation =0;
        Integer shipmentPending = 0;
        Integer receivedPending = 0;
        Integer returnedGoods = 0;
        Integer finished = 0;
        for(Order order : list){
            if(order.getStatus().equals(0)){
                obligation = obligation + Integer.valueOf(order.getCountStatus());
            }
            if(order.getStatus().equals(1)){
                shipmentPending = shipmentPending+Integer.valueOf(order.getCountStatus());
            }
            if(order.getStatus().equals(2)){
                receivedPending = receivedPending+Integer.valueOf(order.getCountStatus());
            }
            if(order.getStatus().equals(3)){
                finished = finished+Integer.valueOf(order.getCountStatus());
            }
            if(order.getStatus().equals(4)){
                returnedGoods = returnedGoods+Integer.valueOf(order.getCountStatus());
            }
        ...
        map.put("obligation",obligation.toString());//待付款
        map.put("shipmentPending",shipmentPending.toString());//待发货
        map.put("receivedPending",receivedPending.toString());//待收货
        map.put("returnedGoods",returnedGoods.toString());//退换货
        map.put("finished",finished.toString());//已完成
        return map;
    }
