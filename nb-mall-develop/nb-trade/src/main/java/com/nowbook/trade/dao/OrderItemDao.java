...
public class OrderItemDao extends SqlSessionDaoSupport {
    ...
    public Paging<Long> findOrderIdsBy(Map<String, String> params, Integer offset, Integer limit) {
        ...
        if (params.containsKey("distributorId")) {
            p.put("distributorId", Long.parseLong(params.get("distributorId")));
        }
    ...
    public Paging<Long> findOrderIdsByForNs(Map<String, String> params, Integer offset, Integer limit) {
        Map<String, Object> p = Maps.newHashMap();
        if (params.containsKey("buyerId")) {
            p.put("buyerId", Long.parseLong(params.get("buyerId")));
        }
        ...
        p.put("offset", offset);
        p.put("limit", limit);
        Long count = getSqlSession().selectOne("OrderItem.countOrderIdsForNs", p);
        if (count == 0) {
            return new Paging<Long>(0L, Collections.<Long>emptyList());
        } else {
            List<Long> orderIds = getSqlSession().selectList("OrderItem.findOrderIdsByForNs", p);
            return new Paging<Long>(count, orderIds);
        }
    }
