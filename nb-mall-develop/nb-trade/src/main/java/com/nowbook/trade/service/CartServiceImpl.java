...
import com.nowbook.trade.dao.CartDao;
import com.nowbook.trade.dto.PreOrder;
import com.nowbook.trade.dto.RichOrderItem;
import com.nowbook.trade.model.CartItem;
import com.nowbook.trade.model.DeliveryMethod;
...
import com.nowbook.coupons.service.NbCouUserService;
...
public class CartServiceImpl implements CartService {
    ...
    private final CouponsNbService couponsNbService;

    private final NbCouUserService nbCouUserService;
    ...
    public CartServiceImpl(ItemService itemService, ShopService shopService, CartDao cartDao,
                           ...
                           ShopExtraService shopExtraService, CouponsNbService couponsNbService, NbCouUserService nbCouUserService, CouponsItemListService couponsItemListService) {
        ...
        this.couponsNbService = couponsNbService;
        this.nbCouUserService = nbCouUserService;
        ...
        try {
            ...
            if (null != userCarts) {
                ...
                for (int i = 0; i < userCarts.size(); i++) {
                    ...
                    List<NbCouponsItemList> couponsList = couponsItemListService.findCouponsbyShopId(shopId);
					for(NbCouponsItemList cous:couponsList){
    ...
    public Response<List<PreOrder>> preOrder(@ParamInfo("baseUser") BaseUser baseUser, String skus) {
        ...
        //获取当前用户未使用的商家优惠券信息
//        Response<List<NbCouUserView>> listResponse = nbCouUserService.queryCouponsAllByUser(baseUser, skus, 1L);
        ...
        try {
            ...
            for (Long sellerId : grouped.keySet()) {
                ...
                //是否是enb商家
                preOrder.setIsEnb(Objects.equal(eNbSellerId, shop.getUserId().toString()));
...
//              * @param itemIds 传入多个ItemId (1,2) 逗号隔开
              /*  String couponStatus = "1,2";
                Response<List<NbCou>> listResponse1 =  couponsNbService.querySellerCouponsByParam(baseUser,1L,couponStatus,queryItemIds);
...
//    }
    ...
    private List<CartItem> buildCartItems(Multiset<Long> skuIds) {
        ...
        for (Long skuId : skuIds.elementSet()) {
            ...
            Integer price=levelPriceService.getUserLevelPrice(sku);
            ...
            cartItem.setSellerId(item.getUserId());
            cartItem.setPriceType(item.getPriceType());
            result.add(cartItem);
        }
        return result;
    }
