...
import com.nowbook.rlt.settle.service.SettlementJobService;
...
//    @Scheduled(cron = "0 0/1 * * * *")
    public void payEarningsBonuses() {
        boolean isLeader = this.adminLeader.isLeader();
        if (!isLeader) {
            log.info("current admin leader is:{}, return redirect", adminLeader.currentLeaderId());
            return;
        ...
        Date doneAt = DateTime.now().withTimeAtStartOfDay().toDate();
        log.info("[PAY-EARNINGS-BONUSES] begin {}", DFT.print(DateTime.now()));
        settlementJobService.payEarningsBonuses(doneAt);
        log.info("[PAY-EARNINGS-BONUSES] DONE");
    }
...
//    @Scheduled(cron = "0 55 14 * * *")
    public void sumEarningsBonuses() {
        boolean isLeader = this.adminLeader.isLeader();
        if (!isLeader) {
            log.info("current admin leader is:{}, return redirect", adminLeader.currentLeaderId());
            return;
        ...
        Date doneAt = DateTime.now().withTimeAtStartOfDay().toDate();
        log.info("[SUM-EARNINGS-BONUSES] begin {}", DFT.print(DateTime.now()));
        settlementJobService.sumEarningsBonuses(doneAt);
        log.info("[SUM-EARNINGS-BONUSES] DONE");
    }
...
//    @Scheduled(cron = "0/15 * * * * *")
    public void createSettlements() {
        ...
        Date doneAt = DateTime.now().withTimeAtStartOfDay().toDate();
        log.info("[CREAT_SETTLEMENTS] begin {}", DFT.print(DateTime.now()));
        settlementJobService.createSettlements(doneAt);
        log.info("[CREAT_SETTLEMENTS] DONE");
    }
...
//        }
//        Date doneAt = DateTime.now().withTimeAtStartOfDay().toDate();
//        log.info("[COMMISSION] doneAt {} }",
//                DFT.print(new DateTime(doneAt)));
//        settlementJobService.commission(doneAt);
//        log.info("[COMMISSION] DONE");
//
//    }
