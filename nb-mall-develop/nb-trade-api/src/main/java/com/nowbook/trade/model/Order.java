...
public class Order implements Serializable {
    ...
    public static enum Status2 {
        ...
        public static Order.Status from(int value) {
            for(Order.Status ta: Order.Status.values()) {
                if(ta.value==value) {
                    return ta;
                }
            }
    ...
    @Getter
    @Setter
    private Integer level;      // 支付平台 1：支付宝 2：微信 3：其他

    @Getter
    @Setter
    private Long shopId;      // 支付平台 1：支付宝 2：微信 3：其他


    //添加时间：2017-7-24   dpzh
    @Getter
    @Setter
    private String countStatus;          //根据status统计各状态的数量


    @Getter
    @Setter
    private Integer isBalance;            //是否可以用余额支付
