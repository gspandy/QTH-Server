...
@ToString
@NoArgsConstructor
public class NbOrder implements Serializable {
    ...
    @Getter
    @Setter
    private String tempReturnId;                    // NB临时逆向订单号

    @Getter
    @Setter
    protected List<NbOrderItem> items = Lists.newLinkedList();    // 订单商品明细





    public static NbOrder transform(Order order, List<OrderItem> orderItems, Long shopId, String outerId) {
        NbOrder dto = new NbOrder();
        List<NbOrderItem> items = BeanMapper.mapList(orderItems, NbOrderItem.class);
    ...
    public static NbOrder transform(Order order, Long shopId, String outerId) {
        NbOrder dto = new NbOrder();
        BeanMapper.copy(order, dto);
        dto.setShopId(shopId);
        dto.setOuterCode(outerId);
    ...
    private static void transDate(NbOrder dto, Order order) {
