...
import com.nowbook.web.dto.JsonValue;
...
public class CouponsWebController {
    ...
    public JsonValue queryItemCouponBy(@RequestParam("itemId") Long itemId){
        ...
        //0 不显示  1 显示
        Response<List<NbCouponsItemList>> listResponse =   couponsItemListService.queryCouponsItemListBy(null,itemId,null,"");
    ...
    public JsonValue queryItemCouponBy(@RequestParam("couponsId") Long couponsId,
        ...
        Response<List<NbCouponsItemList>> listResponse = couponsItemListService.queryCouponsItemListBy(couponsId, null, null, "");
        if(!listResponse.isSuccess()){
            jsonValue.setStatus(0L);
            jsonValue.setMessage("产品不存在,请重新确认");
            return jsonValue;
        ...
        result = couponsNbService.queryCouponsById(couponsId);
        ...
        Iterator<NbCouponsItemList> its =  listResponse.getResult().iterator();
...
//        }
    ...
    @ResponseBody
    public NbCou queryCouponsBy(@RequestParam("couponsId") Long couponsId){
        Response<NbCou> result = new Response<NbCou>();
        result = couponsNbService.queryCouponsById(couponsId);
        if(!result.isSuccess()) {
            log.error("find nbCoupons failed, cause:{}", result.getError());
    ...
    @ResponseBody
    public Response<Paging<NbCou>> getCouponsBySellerId(@RequestParam("sellerId") Long sellerId, Long pageIndex, Long pageSize) {
    	return couponsNbService.queryCouponsByShopId(sellerId, pageIndex, pageSize);
    }
...
//        }
//
//        NbCou nbCouObj =  result.getResult();
//        //判断 区域是否在该优惠券 使用范围 内
//        String area = nbCouObj.getArea();
...
//        Integer totalPrice = 0;
//        int couponsTerm = nbCouObj.getTerm();
//        Response<List<NbCouponsExceptSpus>> couponsExceptSpusList =  couponsNbService.querySpuListByCouponsId(couponsId);
//        if(couponsExceptSpusList.isSuccess()){
//            List<NbCouponsExceptSpus> couponsExceptList = couponsExceptSpusList.getResult();
//            for(int ec=0;ec<couponsExceptList.size();ec++){
//                NbCouponsExceptSpus couponsExceptSpus = couponsExceptList.get(ec);
