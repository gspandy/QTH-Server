...
public class CollectedItems {
    ...
    @ResponseBody
    public CollectedBar collected(@PathVariable Long itemId) {
        try {
            Response<CollectedBar> res = collectedItemService.collected(itemId, UserUtil.getUserId());
    ...
    @ResponseBody
    public CollectedSummary create(@PathVariable Long itemId) {

        try {
            checkArgument(notNull(itemId), "item.id.can.not.be.empty");
            Response<CollectedSummary> res = collectedItemService.create(UserUtil.getUserId(), itemId);
            checkState(res.isSuccess(), res.getError());
            return res.getResult();
    ...
    public String delete(@PathVariable Long itemId) {
        ...
        try {
            checkArgument(notNull(itemId), "item.id.can.not.be.empty");
            Response<Boolean> res = collectedItemService.delete(UserUtil.getUserId(), itemId);
            checkState(res.isSuccess(), res.getError());
            return "ok";
