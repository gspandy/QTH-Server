...
import com.nowbook.item.model.ItemWithTags;
...
public class SellerCouponsController {
     ...
     Response<String> result = new Response<String>();
      ...
      if(baseUser==null){
        result.setResult("404");
          ...
          List<NbCou> list  = couponsManageService.findAllNbCou(userId,pageCount);
          StringBuilder stb = new StringBuilder();
          if(list!=null&&list.size()!=0){
              for(NbCou nbCou:list){
                    int tempTerm = nbCou.getTerm()/100;
                    int tempAmount = nbCou.getAmount()/100;
                    ...
                    String sen= nbCou.getSendNum()==0?"不限":String.valueOf(nbCou.getSendNum());
                    String status = nbCou.getStatus()==0?"未生效":nbCou.getStatus()==1?"暂停":nbCou.getStatus()==2?"生效":nbCou.getStatus()==3?"失效":nbCou.getStatus()==4?"撤销":"获取状态失败";
                    stb.append("<tr><td><p><a href=\"javascript:void(0)\" class=\"couponname\">"+nbCou.getCpName()+"</a></p>");
                    stb.append("<div class=\"coupon_popup\"><h3>优惠券设置信息<a href=\"javascript:void(0)\" class=\"close_popup\">X</a></h3>");
                    stb.append("<div class=\"coupon_popup_content\"><dl><dt>券名称</dt><dd>"+nbCou.getCpName()+"</dd></dl><dl><dt>有效期</dt><dd>"+startDate+"至"+endDate+"</dd></dl><dl><dt>领券URL</dt><dd>http://"+domainUrl+"/seller/seller-coupon-detail?couponsId="+nbCou.getId()+"</dd></dl>");
                    stb.append("<dl><dt>优惠条件[满]</dt><dd>"+tempTerm+"元</dd></dl><dl><dt>优惠条件[减]</dt><dd>"+tempAmount+"元</dd></dl>");
                    stb.append("<dl><dt>发券数量</dt><dd>"+sen+"</dd></dl><dl><dt>ID限领数量</dt><dd>"+nbCou.getUseLimit()+"</dd></dl><dl><dt>商品范围</dt><dd>限以下分类商品：<p>"+nbCou.getCategoryName()+"</p>部分或全部商品使用</dd></dl>");
                    stb.append("<dl><dt>备注</dt><dd>"+nbCou.getMemo()+"</dd></dl></div></div></td>");
                    stb.append("<td>满"+tempTerm+"元减"+tempAmount+"元</td><td class=\"tl\">"+startDate+"至"+endDate+"</td>");
                    stb.append("<td>"+createAt+"</td>");
                    stb.append("<td>"+nbCou.getSendNum()+"/"+nbCou.getCouponReceive()+"/<a href=\"/seller/coupons_item_list?couponsId="+nbCou.getId()+"\">"+nbCou.getCouponUse()+"</a></td>");
                    stb.append("<td>"+status+"</td>");
                    if(nbCou.getStatus()==0) {
                        stb.append("<td><a href=\"/seller/sellerCouponEdit?couponId="+nbCou.getId()+"\" class=\"operate\">编辑</a></td></tr>");
                    }else if(nbCou.getStatus()==2){
                        stb.append("<td><a href=\"/api/SCoupons/stopCoupons?couponId="+nbCou.getId()+"\" class=\"operate\">中止</a></td></tr>");
                    }else{
                        stb.append("<td></td></tr>");
                    }
              }
    ...
    public Response<String> countCouponsSearch(@RequestParam("endAt") String endAt,@RequestParam("startAt") String startAt,@RequestParam("couponName") String couponName,@RequestParam("selector") String selector){           Response<String> result = new Response<String>();
        NbCou nbCou2 = new NbCou();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        if(!"nulls".equals(endAt)){
            try {
                nbCou2.setEndTime(sdf.parse(endAt));
        ...
        if(!"nulls".equals(startAt)){
            try {
                nbCou2.setStartTime(sdf.parse(startAt));
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
        if(!"nulls".equals(couponName)){
            nbCou2.setCpName(couponName);

        }
        nbCou2.setStatus(Integer.parseInt(selector));
        BaseUser baseUser = UserUtil.getCurrentUser();
        nbCou2.setSellerId(baseUser.getId());
        Integer count = couponsManageService.countCouBySearch(nbCou2);
        result.setResult("共（"+count+"）类优惠券");
        return result;
    }
        ...
        public Response<String> queryCouponsBySearrch(@RequestParam("endAt") String endAt,@RequestParam("startAt") String startAt,@RequestParam("couponName") String couponName,@RequestParam("selector") String selector,@RequestParam("pageCount") int pageCount) {
            ...
            List<NbCou> list  = couponsManageService.findAllBySearch(map);
            StringBuilder stb = new StringBuilder();
            if(list!=null&&list.size()!=0) {
                for (NbCou nbCou : list) {
                    int tempTerm = nbCou.getTerm() / 100;
                    ...
                    stb.append("<div class=\"coupon_popup_content\"><dl><dt>券名称</dt><dd>" + nbCou.getCpName() + "</dd></dl><dl><dt>有效期</dt><dd>" + startDate + "至" + endDate + "</dd></dl><dl><dt>领券URL</dt><dd>http://"+domainUrl+"/seller/seller-coupon-detail?couponsId="+nbCou.getId()+"</dd></dl>");
                    stb.append("<dl><dt>优惠条件[满]</dt><dd>" + tempTerm + "元</dd></dl><dl><dt>优惠条件[减]</dt><dd>" + tempAmount + "元</dd></dl>");
                    stb.append("<dl><dt>发券数量</dt><dd>" + nbCou.getSendNum() + "</dd></dl><dl><dt>ID限领数量</dt><dd>" + nbCou.getUseLimit() + "</dd></dl><dl><dt>商品范围</dt><dd>限以下分类商品：<p>" + nbCou.getCategoryName() + "</p>部分或全部商品使用</dd></dl>");
                    stb.append("<dl><dt>备注</dt><dd>" + nbCou.getMemo() + "</dd></dl></div></div></td>");
                    stb.append("<td>满" + tempTerm + "元减" + tempAmount + "元</td><td class=\"tl\">" + startDate + "至" + endDate + "</td>");
                    stb.append("<td>" + createAt + "</td>");
                    stb.append("<td>" + nbCou.getSendNum() + "/" + nbCou.getCouponReceive() + "/<a href=\"/seller/coupons_item_list?couponsId="+nbCou.getId()+"\">" + nbCou.getCouponUse() + "</a></td>");
    ...
    public void addCoupon(HttpServletRequest request) {
        ...
        int couponsId=couponsNbService.addCoupon(paramMap);
        ...
        couponsNbService.insertItemIds(listMap);
    }
    ...
    public void updateCoupon(HttpServletRequest request) {
        ...
        Response<Boolean> bool= couponsNbService.deleteCouponsId(id);
        if(bool.getResult()){
            BaseUser baseUser=UserUtil.getCurrentUser();
            Shop shop=shopService.findByUserId(baseUser.getId()).getResult();
            Map<String, Object> paramMap = new HashMap<String, Object>();
        ...
        /*平台优惠券需要的参数，不加会报错，是因为用的平台优惠券的添加方法*/
            ...
            String couponsCode="SP"+getDate();
            paramMap.put("couponsCode",couponsCode);
            couponsNbService.updateCoupon(paramMap);
            List<Map<String,Object>> listMap=new ArrayList<Map<String, Object>>();
            String itemIdStr= request.getParameter("itemIdStr");
            String itemTagStr= request.getParameter("itemTagStr");
            String [] itemTags=itemTagStr.split(",");
    ...
    public Response<Map<String,Object>> findEditCouponsById(HttpServletRequest request) {
       ...
       Map<String,Object> map=new HashMap<String, Object>();
       NbCou nbCou= couponsManageService.findEditById(Long.parseLong(request.getParameter("id")));
       DateFormat dateFormat=new SimpleDateFormat("yyyy-MM-dd hh");
       map.put("id",nbCou.getId());
       map.put("cpName",nbCou.getCpName());
       map.put("startTime",dateFormat.format(nbCou.getStartTime()));
       map.put("endTime",dateFormat.format(nbCou.getEndTime()));
       map.put("term",nbCou.getTerm()/100);
       map.put("amount",nbCou.getAmount()/100);
       map.put("sendNum",nbCou.getSendNum());
       map.put("useLimit",nbCou.getUseLimit());
       map.put("memo",nbCou.getMemo());
       map.put("nameStr",nbCou.getCategoryName());
    ...
    public Response<List<ItemWithTags>> findEditItems(HttpServletRequest request) {
        ...
        List<Map<String,Object>> listMap=couponsNbService.findEditItems(request.getParameter("id"));
