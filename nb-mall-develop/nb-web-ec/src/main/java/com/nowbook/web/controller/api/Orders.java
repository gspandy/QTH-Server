...
import com.nowbook.alipay.mpiPay.MPIRequest;
...
import com.nowbook.rlt.buying.service.BuyingActivityDefinitionService;
import com.nowbook.rlt.buying.service.BuyingItemService;
import com.nowbook.rlt.buying.service.BuyingOrderRecordService;
import com.nowbook.rlt.buying.service.BuyingTempOrderService;
...
import com.nowbook.rlt.predeposit.dto.FatOrderPreDeposit;
...
import com.nowbook.web.controller.api.userEvent.*;
...
import static com.nowbook.rlt.settle.util.SettlementVerification.*;
...
public class Orders {
    ...
    @Autowired
    PushOrderEventBus pushOrderEventBus;
        ...
        try {
            ...
            if(payType==1){
                if(status==-3){
                    ...
                    pushOrderEventBus.post(new PushEvent(orderItem.getOrderId(),orderItem.getId(),NoticeType.ORDER_APPLY_REFUND.getNoticeType()));
                }else if(status==-4){
                    smsEventBus.post(new SmsEvent(orderItem.getOrderId(),orderItem.getId(),"7"));
                    pushOrderEventBus.post(new PushEvent(orderItem.getOrderId(),orderItem.getId(),NoticeType.ORDER_RETURNED_PURCHASE.getNoticeType()));
            		/*if(isUserStatus(String.valueOf(buyerId),"1")){
    ...
    public String deliver(@PathVariable("orderId") Long orderId, @RequestBody OrderLogisticsInfoDto orderLogisticsInfoDto) {
        ...
        try {
            ...
            if(type==1){
                ...
                pushOrderEventBus.post(new PushEvent(orderId, orderId, NoticeType.ORDER_SHIPPED.getNoticeType()));
...
//            createSettlementAfterConfirm(order);
    ...
    public Response collect(@RequestParam("orderId") Long orderId,
        ...
        try {
            ...
            order.setPaymentPlatform("3");
//            Response<Boolean> result = orderWriteService.normalOrderPaid(order);
//            if (!result.isSuccess()) {
...
//            log.info("succeed to confirm collection for order(id={}) by user(id={})");
        ...
        response.setHeader("Access-Control-Allow-Origin", "http://home.nowbook.cn/*");
        return ret;
    }
...
//    }
    ...
    public String refuse(@PathVariable("orderItemId") Long orderItemId) {
        ...
        if (orderItemR.isSuccess()) {
            orderItem = orderItemR.getResult();
            status=orderQueryService.findOrderItemById(orderItemId).getResult().getStatus();
        ...
        if (!result.isSuccess()) {
            ...
            if(payType==1){
                if(status==4){
                //申请退款
                    pushOrderEventBus.post(new PushEvent(orderItem.getOrderId(),orderItem.getId(),NoticeType.ORDER_APPLY_REFUND_NOT_PASS.getNoticeType()));
                }else if(status==5){
                //申请退货
                    pushOrderEventBus.post(new PushEvent(orderItem.getOrderId(),orderItem.getId(),NoticeType.ORDER_RETURNED_PURCHASE_NOT_PASS.getNoticeType()));
                }
            }
        }
        return "ok";
    ...
    public Paging<RichOrderBuyerView> viewBuyerOrders(@RequestParam(value = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        Response<Paging<RichOrderBuyerView>> orderGet =  orderQueryService.findByBuyerId(currentBuyer, pageNo, size, status, orderId,null);
    ...
    private void addOrderCodeUsageInfo(RichOrderBuyerView richOrderBuyerView, CodeUsage codeUsage) {
        ...
        List<NbCouOrderItem> couponList = richOrderBuyerView.getCouponList();
        if (null == couponList || couponList.size() < 1) {
            couponList = new ArrayList<NbCouOrderItem>();
        }
        for(RichOrderItem item : richOrderBuyerView.getOrderItems()) {
            NbCouOrderItem couOrder = new NbCouOrderItem();
            ...
            for (RichOrderSellerView richOrderSellerView : result.getData()) {
                ...
                if (codeUsage != null && codeUsage.getResult() != null) {
                    ...
                    if (equalWith(richOrderSellerView.getIsBuying(),Boolean.TRUE)) {
                        addOrdeNBellerCodeUsageInfo(richOrderSellerView, codeUsage.getResult());
                    }
                }
            }
            addBuyingInfo(result.getData());
...
//          add by zf 2014-08-22
     ...
     * @param codeUsage
     */
    private void addOrdeNBellerCodeUsageInfo(RichOrderSellerView richOrderSellerView, CodeUsage codeUsage) {
        log.debug("init order sell coupon info begin ...");
        List<NbCouOrderItem> couponList = richOrderSellerView.getCouOrderList();
        if (null == couponList || couponList.size() < 1) {
            couponList = new ArrayList<NbCouOrderItem>();
        }
...
//                    }
    ...
    private void addOrderCouponInfo(List<RichOrderBuyerView> data) {
        ...
        List<NbCouOrderItem> orderCouponList = couponsNbService
        ...
        // 初始化订单的优惠券使用情况
        Map<String, NbCouOrderItem> orderCouponMap = new HashMap<String, NbCouOrderItem>();

        StringBuffer buff = new StringBuffer();
        for (NbCouOrderItem couOrder : orderCouponList) {
            getOrderCouponKey(buff, couOrder.getOrderId() , couOrder.getItemId() , couOrder.getSkuId());
            orderCouponMap.put(buff.toString(), couOrder);
        }

        for (RichOrderBuyerView order : data) {
            List<NbCouOrderItem> couponList = order.getCouponList();
            if (null == couponList||couponList.size()<1) {
                couponList = new ArrayList<NbCouOrderItem>();
            }
            ...
            for(RichOrderItem item : order.getOrderItems()){
                ...
                NbCouOrderItem couOrder = orderCouponMap.get(buff.toString());
                if (null != couOrder) {
    ...
    private void addCouponsSaleOrderinfo(List<RichOrderSellerView> data) {
        ...
        for (RichOrderSellerView order : data) {
            List<NbCouOrderItem> couponList = order.getCouOrderList();
            if (null == couponList||couponList.size()<1) {
                couponList = new ArrayList<NbCouOrderItem>();
            }
...
//                        Response<Long>  resultValue =  couponsNbService.checkJoin(orderItem.getItemId());//判断该产品是否参加优惠分配
                ...
                log.info("nbCou.getAmount()"+nbCou.getAmount());
                log.info(total+"totalCoupons"+totalCoupons);
                //盘旋选择的优惠券是否可以使用
                if(nbCou.getTerm() > totalCoupons){//不能使用返回原价
                    return total;
                }
    ...
    private int getsellerCouonsByUse(List<FatOrder> fatOrders,int total,Set<Long> ids,Long userId){
        ...
        for (Long id : ids) {
            ...
            if(sellerCouponMap.containsKey(order.getSellerId())){ //判断该店铺是否选择了优惠券
                ...
                ///////////////////////////////计算参加优惠券的产品的价格之和/////////////////////////////////////
                /******************************************************************/
                Response<List<NbCouponsItemList>> couponsItemListBy = couponsItemListService.queryCouponsItemListBy(couponsId, null, null, "");
                Iterator<NbCouponsItemList> its =  couponsItemListBy.getResult().iterator();
                HashMap<String,NbCouponsItemList> allItemMap = new HashMap<String,NbCouponsItemList>();
                while(its.hasNext()){
                    NbCouponsItemList nbCouponsItemList = its.next();
                    String keyCode = String.valueOf(nbCouponsItemList.getItemId());
                    allItemMap.put(keyCode,nbCouponsItemList);
                }
                /******************************************************************/
...
//                while(its.hasNext()){
//                    NbCouponsItemList nbCouponsItemList = its.next();
//                    Long keyCode = nbCouponsItemList.getItemId();
...
//                }
                ///////////////////////////////////计算参加优惠券的产品的价格之和/////////////////////////////////////////////////
                Response<NbCou> nbCouResponse = new Response<NbCou>();
                log.info("====couponsId====="+couponsId);
                nbCouResponse = couponsNbService.queryCouponsById(couponsId);
                if(nbCouResponse.isSuccess() && nbCouResponse.getResult()!=null){
                    int couponsTerm = nbCouResponse.getResult().getTerm();
                    ...
                    if((totalPrice.intValue()) >= couponsTerm){  //产品价格大于优惠券的最低消费价格 则可以使用 否则不能使用
                        ...
                        int couponsAmount = nbCouResponse.getResult().getAmount();
                        ...
                        //修改已使用优惠券信息 couponUse
                        NbCou nbcouObj = nbCouResponse.getResult();
                        nbcouObj.setCouponUse(nbcouObj.getCouponUse()+1);
                        couponsNbService.updateNbCou(nbcouObj);

                        Response<NbCouUser> resutUser = couponsNbService.queryCouponsUserBy(userId,couponsId);
                        if(resutUser.isSuccess()){
                            NbCouUser nbCouUser = resutUser.getResult();
                            log.info("init nbCouUser end ...coupuons"+nbCouUser.getId());
                            couponsNbService.updateCouponUser(nbCouUser.getId());
