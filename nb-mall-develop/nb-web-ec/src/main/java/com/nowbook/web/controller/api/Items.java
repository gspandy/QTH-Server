...
import com.nowbook.trade.dto.FreightModelDto;
import com.nowbook.trade.model.FreightModel;
import com.nowbook.trade.service.FreightModelService;
...
//        item.setRegion(joiner.join(regionIds));
        ...
        //通过spuId获取品牌id,时间：2017-09-26 dpzh
        Response<Spu> spuR = spuService.findById(itemDto.getItem().getSpuId());
        if(!spuR.isSuccess()) {
            log.error("query spu fail,spuId={},error code:{}",itemDto.getItem().getSpuId(), spuR.getError());
