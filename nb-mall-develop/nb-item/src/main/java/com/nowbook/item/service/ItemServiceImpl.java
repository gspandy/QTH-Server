...
import com.nowbook.category.service.Forest;
...
import com.nowbook.item.dao.mysql.ItemServiceDao;
import com.nowbook.item.dao.mysql.SkuDao;
import com.nowbook.item.dao.redis.ItemCountDao;
import com.nowbook.item.dao.redis.ItemDetaiImageDao;
import com.nowbook.item.dao.redis.ItemsTagsDao;
import com.nowbook.item.dto.FullDefaultItem;
import com.nowbook.item.dto.FullItem;
import com.nowbook.item.dto.RichSpu;
import com.nowbook.item.dto.SkuGroup;
import com.nowbook.item.event.DspEvent;
import com.nowbook.item.event.ItemCountEvent;
import com.nowbook.item.event.ItemEventBus;
import com.nowbook.item.manager.ItemManager;
import com.nowbook.item.manager.ItemRealTimeIndexer;
...
//        id=88L;
        ...
        try {
            ...
            //enb商家
            if(Objects.equal(eNbSellerId,item.getUserId().toString())){
                richSpu.setIsEnb(Boolean.TRUE);
            }else {
                richSpu.setIsEnb(Boolean.FALSE);
            }
...
//        id=88L;
    ...
    public Response<Long> create(Item item, ItemDetail itemDetail, List<Sku> skus, Boolean needValidate) {
        ...
        if (item.getPriceType() == null) {
            ...
            result.setError("item.priceType.null");
            return result;
        }
        ...
        try {
            ...
            if(item.getPriceType().equals(1)){
                ...
                for (Sku sku : skus) {
                    ...
                    sku.setPriceType(item.getPriceType());
                    lowPrice = lowPrice < sku.getPrice() ? lowPrice : sku.getPrice();
                }
                item.setPrice(lowPrice);
            }
            if(item.getPriceType().equals(2)){
                int lowPrice1 = skus.get(0).getSellingPrice();
                int lowPrice2 = skus.get(0).getPurchasePrice();
                int i = 0;
                for (Sku sku : skus) {
                    if(sku.getSellingPrice() == null || sku.getPurchasePrice() == null || sku.getAngelRecommendPrice() == null) {
                        log.error("sku{} price can not be null", sku);
                        result.setError("sku.price.null");
                        return result;
                    }
                    sku.setPriceType(item.getPriceType());
                    lowPrice1 = lowPrice1 < sku.getSellingPrice() ? lowPrice1 : sku.getSellingPrice();
                    lowPrice2 = lowPrice2 < sku.getPurchasePrice() ? lowPrice2 : sku.getPurchasePrice();
                }
                item.setSellingPrice(lowPrice1);
                item.setPurchasePrice(lowPrice2);
            }
            if(item.getPriceType().equals(3)){
                int lowPrice0 = skus.get(0).getCustomPrice();
                int lowPrice1 = skus.get(0).getAngelPrice();
                int lowPrice2 = skus.get(0).getGoldPrice();
                int lowPrice3 = skus.get(0).getPlatinumPrice();
                int lowPrice4 = skus.get(0).getBlackPrice();
                int lowPrice5 = skus.get(0).getPartnerPrice();
                int i = 0;
                for (Sku sku : skus) {
                    if(sku.getCustomPrice() == null || sku.getAngelPrice() == null || sku.getGoldPrice() == null || sku.getPlatinumPrice() == null || sku.getBlackPrice() == null ||sku.getPartnerPrice() == null|| sku.getAngelRecommendPrice() == null ) {
                        log.error("sku{} price can not be null", sku);
                        result.setError("sku.price.null");
                        return result;
                    }
                    sku.setPriceType(item.getPriceType());
                    lowPrice0 = lowPrice0 < sku.getCustomPrice() ? lowPrice0 : sku.getCustomPrice();
                    lowPrice1 = lowPrice1 < sku.getAngelPrice() ? lowPrice1 : sku.getAngelPrice();
                    lowPrice2 = lowPrice2 < sku.getGoldPrice() ? lowPrice2 : sku.getGoldPrice();
                    lowPrice3 = lowPrice3 < sku.getPlatinumPrice() ? lowPrice3 : sku.getPlatinumPrice();
                    lowPrice4 = lowPrice4 < sku.getBlackPrice() ? lowPrice4 : sku.getBlackPrice();
                    lowPrice5 = lowPrice5 < sku.getPartnerPrice() ? lowPrice5 : sku.getPartnerPrice();
                }
                item.setCustomPrice(lowPrice0);
                item.setAngelPrice(lowPrice1);
                item.setGoldPrice(lowPrice2);
                item.setPlatinumPrice(lowPrice3);
                item.setBlackPrice(lowPrice4);
                item.setPartnerPrice(lowPrice5);
            }
...
//                }
                if(item.getPriceType().equals(2)){
                    for (Sku sku : skus) {
                        if(sku.getPurchasePrice()>sku.getSellingPrice()){
                            result.setError("item.sellingPrice.error");
                            return result;
                        }
                    }
                }
                if(item.getPriceType().equals(3)){
                    for (Sku sku : skus) {
                        if(sku.getCustomPrice()<sku.getAngelPrice()){
                            result.setError("item.jt.error");
                            return result;
                        }
                        if(sku.getAngelPrice()<sku.getGoldPrice()){
                            result.setError("item.jt.error");
                            return result;
                        }
                        if(sku.getGoldPrice()<sku.getPlatinumPrice()){
                            result.setError("item.jt.error");
                            return result;
                        }
                        if(sku.getPlatinumPrice()<sku.getBlackPrice()){
                            result.setError("item.jt.error");
                            return result;
                        }
                        if(sku.getBlackPrice()<sku.getPartnerPrice()){
                            result.setError("item.jt.error");
                            return result;
                        }
                    }
...
//        itemId=88L;
        ...
        ShopExtra shopExtra = shopExtraDao.findByShopId(item.getShopId());
        if(shopExtra !=null){
            item.setNtalkerId(shopExtra.getNtalkerId());
        }
        //通过item获取对应用户等级的商品价格 2017-09-27 dpzh
        ...
        try {
            ...
            for(Sku sku : skus){
                //通过sku获取对应用户等级的商品价格 2017-09-27 dpzh
            ...
            String picture = itemDetaiImageDao.getItemIdEvaluation(itemId);
            if(picture == null || picture .equals("")){
                picture =itemDetaiImageDao.getSpuIdEvaluation(item.getSpuId());
            }
     ...
     */
    @Override
    public Response<Map<String, Object>> findWithDetailsByIdForCustomer(Long itemId,Long userId) {
...
//        itemId=88L;
        ...
        //通过item获取对应用户等级的商品价格 2017-09-27 dpzh
        item.setSalePrice(levelPriceService.getUserLevelPriceForCustomer(item,userId));
        Integer level = -1;
        if(userId !=null){
            List<Level> levels=levelService.selectByUserId(userId);
            if(levels!=null && levels.size()>0){
                level = levels.get(0).getLevel();
            }
        }
        ...
        Response<Brand> brandResponse = brandService.findById(item.getBrandId());
        String category = "";
        for(Pair pair : pairs){
            category =  category+"→"+ pair.getName();
        }
        if(!category.equals("")){
        ...
        try {
            ...
            map.put("imageurl",item.getMainImage());
            if(brandResponse.getResult() !=null){
                map.put("brand",brandResponse.getResult().getName());
            }
            map.put("category",category);
            map.put("marketprice",Double.valueOf(item.getPrice())/100);
            map.put("siteprice",Double.valueOf(item.getSalePrice())/100);
            map.put("currency","¥");
            result.setResult(map);
            return result;
        } catch (Exception e) {
