...
public class BrandUserDao extends SqlSessionDaoSupport {
     ...
     * @return
     */
    public int brandIsExist(NbBrand brand){
        return (Integer)getSqlSession().selectOne("NbBrand.isExistsBrand",brand);
    }
    /**
     * 审核成功改变状态
     */
    public boolean updateSucc(BrandClub brandClub,NbBrand brand){
        int count = (Integer)getSqlSession().selectOne("NbBrand.countBrand",brand);
        if(count==0){
            return false;
        }else {
            NbBrand brand2 = getSqlSession().selectOne("NbBrand.findBrandId", brand);
