package com.nowbook.web.interceptor;

import com.nowbook.exception.JsonResponseException;
import com.nowbook.site.exception.UnAuthorize401Exception;
...
public class AuthInterceptor extends HandlerInterceptorAdapter {
    ...
    public AuthInterceptor(String mainSite) throws Exception {
        ...
        Yaml yaml = new Yaml();
        Auths auths = yaml.loadAs(Resources.toString(Resources.getResource("authorize.yaml"), Charsets.UTF_8), Auths.class);
            ...
            if (auth.types != null && !auth.types.isEmpty()) {
                for (String type : auth.types) {
                    if (Objects.equal("ALL", type.toUpperCase())) { //if all, means everyone(if login in)  can access this url
                        for (BaseUser.TYPE t : BaseUser.TYPE.values()) {
                            types.add(t.toNumber());
                        }
                    } else {
                        types.add(BaseUser.TYPE.fromName(type).toNumber());
                    }
                }
            }
    ...
    private void redirectToLogin(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        UriComponents uriComponents =
                UriComponentsBuilder.fromUriString("http://" + mainSite + "/login?target={target}").build();
