...
import com.nowbook.rlt.settle.dao.BusinessRateDao;
...
//                if (equalWith(settlement.getFinished(), Settlement.Finished.NOT.value())) continue;     // 未结束的订单不处理
    ...
    private void doSettlement(Settlement settlement) {
        ...
        Long nbCommission = 0L;
        Long scoreEarning = 0L;
        Long presellDeposit = 0L;

        Long thirdPartyCommission = 0L;
        ...
        for (ItemSettlement sub : subs) {  //计算子订单结算金额
            ...
            nbCommission += sub.getNbCommission();                // 累计平台佣金
...
//        }
        ...
        settlement.setNbCommission(nbCommission);
...
//                nbCommission =   Math.round(totalEarning * rate);  // 营收金额 x 佣金润点
//            }
...
//                nbCommission =  Math.round(sub.getFee() * rate);  // 货款 x 佣金润点
//            }
        }

        Long sellerEarning = totalEarning - totalExpenditure - scoreEarning - nbCommission;    // 商户收入
        sub.setTotalEarning(totalEarning);
        sub.setTotalExpenditure(totalExpenditure);
        sub.setSellerEarning(sellerEarning);
        sub.setNbCommission(nbCommission);
    ...
    private void calculatePresellDeposit(ItemSettlement sub) {
        ...
        if (Objects.equal(sub.getTradeStatus(), Settlement.TradeStatus.DONE.value())) {  // 成功单
            nbCommission = Math.round(sub.getFee() * rate);   //计算佣金 = 货款 *  扣点
        ...
        Long sellerEarning = totalEarning - totalExpenditure - scoreEarning  - nbCommission;   // 计算商户收入
        sub.setTotalEarning(totalEarning);
        sub.setTotalExpenditure(totalExpenditure);
        sub.setSellerEarning(sellerEarning);
        sub.setNbCommission(nbCommission);
    ...
    private void calculatePresellRest(ItemSettlement sub) {
        ...
        if (Objects.equal(sub.getTradeStatus(), Settlement.TradeStatus.DONE.value())) {  // 成功单
            ...
            nbCommission = Math.round(sub.getFee() * rate);   //计算佣金 = 尾款 *  扣点
        ...
        sellerEarning = totalEarning - totalExpenditure - scoreEarning - nbCommission;
        sub.setTotalEarning(totalEarning);
        sub.setTotalExpenditure(totalExpenditure);
        sub.setSellerEarning(sellerEarning);
        sub.setNbCommission(nbCommission);
