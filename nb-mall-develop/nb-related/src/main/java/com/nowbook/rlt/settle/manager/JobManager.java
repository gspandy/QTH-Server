...
//        settleJobDao.create(autoConfirmJob);
        ...
        if(cal.get(Calendar.DAY_OF_WEEK)==Calendar.MONDAY){
            SettleJob sumEarningsBonusesJob = SettleJob.sumEarningsBonuses(doneAt, tradedAt);               // 任务15:周日结算数据
//        reportJob.setDependencyId(autoConfirmJob.getId());
            settleJobDao.create(sumEarningsBonusesJob);
        }

        if(cal.get(Calendar.DAY_OF_WEEK)==Calendar.WEDNESDAY){
            SettleJob payEarningsBonusesJob = SettleJob.payEarningsBonuses(doneAt, tradedAt);               // 任务16:周三结算数据
//        reportJob.setDependencyId(autoConfirmJob.getId());
            settleJobDao.create(payEarningsBonusesJob);
        }
        SettleJob reportJob = SettleJob.report(doneAt, tradedAt);               // 任务2:生成日汇总
//        reportJob.setDependencyId(autoConfirmJob.getId());
        settleJobDao.create(reportJob);

        SettleJob createSettlementsJob = SettleJob.createSettlements(doneAt, tradedAt);               // 任务14:创建结算数据任务
        settleJobDao.create(createSettlementsJob);
