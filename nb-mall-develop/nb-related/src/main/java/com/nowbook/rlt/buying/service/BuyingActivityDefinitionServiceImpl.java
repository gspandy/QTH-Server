package com.nowbook.rlt.buying.service;
...
import com.nowbook.rlt.buying.dto.BuyingPreOrder;
import com.nowbook.rlt.buying.manager.BuyingActivityManger;
...
import com.nowbook.trade.dto.BuyingFatOrder;
import com.nowbook.trade.dto.RichOrderItem;
import com.nowbook.trade.dto.SkuAndItem;
...
public class BuyingActivityDefinitionServiceImpl implements BuyingActivityDefinitionService{
    ...
    public Response<BuyingActivityDto> fingBuyingActivityDtoById(@ParamInfo("id") @Nullable String id,
        ...
        try {
            ...
            log.info("valid isenb userId ={}",userId);
            log.info("valid isenb eNbSellerId ={}",eNbSellerId);
            if(Objects.equal(eNbSellerId,userId.toString())){
                buyingActivityDto.setIsEnb(Boolean.TRUE);
            }else {
                buyingActivityDto.setIsEnb(Boolean.FALSE);
            }
    ...
    private BuyingPreOrder makeBuyingPreOrder(Shop shop, RichOrderItem roi, BuyingTempOrder tempOrder,
        ...
        buyingPreOrder.setIsEnb(Objects.equal(eNbSellerId,shop.getUserId().toString()));
    ...
    private OrderExtra makeOrderExtra(Long buyerId, String buyerNotes, Integer invoiceType,
        ...
        if (notEmpty(buyerNotes) || invoiceType != null || notEmpty(deliverTime)) {
            ...
            //没有发票信息，如果家电频道默认创建
            if (invoiceType == null) {
                if (equalWith(businessId, Business.GZMH.value())) {
        ...
        } else if (equalWith(businessId, Business.GZMH.value())) {  // 家电频道默认创建个人发票
