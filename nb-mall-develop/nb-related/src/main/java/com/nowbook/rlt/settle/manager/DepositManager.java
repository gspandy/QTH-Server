...
public class DepositManager {
    ...
    public Long createDepositFee(DepositFee fee) {
        ...
        // 当支付方式为支付宝时需要创建提现单据
        if (equalWith(fee.getType(), DepositFee.Type.INCREMENT.value()) || equalWith(fee.getType(), DepositFee.Type.TECH_SERVICE.value())) {
    ...
    private void createDepositCash(DepositFee fee, Long id) {
        if (equalWith(fee.getPaymentType(), DepositFee.PaymentType.ALIPAY.value())) {
            ...
            Integer cashType = Objects.equal(fee.getType(), DepositFee.Type.INCREMENT.value()) ?
    ...
    public Long updateDeposit(DepositFee origin, DepositFee updating) {
        ...
        if (equalWith(origin.getPaymentType(), DepositFee.PaymentType.ALIPAY.value())) { // 支付宝提现需要同步修改提现单据
        ...
        if (DepositFee.isTechService(origin)) {   // 技术服务费不用更新账户
        ...
        if (DepositFee.isIncrement(origin)) { // 新增
            balance += delta;
        } else if (DepositFee.isDeduction(origin) || DepositFee.isRefund(origin)) {
            balance -= delta;
        } else {
