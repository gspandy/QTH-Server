 ...
 */
@Repository
public class PreSaleRedisDao {

    private final JedisTemplate jedisTemplate;

    @Autowired
    public PreSaleRedisDao(JedisTemplate jedisTemplate) {
    ...
    public void addPreSaleToAllReleasePreSaleList(final long preSaleId) {
        jedisTemplate.execute(new JedisTemplate.JedisActionNoResult() {
            ...
            public void action(Jedis jedis) {
                jedis.rpush(KeyUtils.allReleasePreSales(), String.valueOf(preSaleId));
            }
        });
    }
    ...
    public List<Long> findAllPreSale() {
        return Lists.transform(jedisTemplate.execute(new JedisTemplate.JedisAction<List<String>>() {
            ...
            public List<String> action(Jedis jedis) {
                return jedis.lrange(KeyUtils.allReleasePreSales(), 0, -1);
            }
    ...
    public void removePreSaleById(final long preSaleId) {
        jedisTemplate.execute(new JedisTemplate.JedisActionNoResult() {
            @Override
            public void action(Jedis jedis) {
                jedis.lrem(KeyUtils.allReleasePreSales(), 1, String.valueOf(preSaleId));
            }
        });
    }
