-- -----------------------------------------------------
-- Table `nb_settlements` 订单结算表
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_settlements`;

CREATE TABLE IF NOT EXISTS `nb_settlements` (
  ...
  `nb_commission`          BIGINT      NULL        COMMENT '平台佣金收入,以分为单位',
...
CREATE INDEX idx_rs_seller_id ON nb_settlements (seller_id);
CREATE INDEX idx_rs_settled_at ON nb_settlements (settled_at);
CREATE INDEX idx_rs_finished_at ON nb_settlements (finished_at);
CREATE INDEX idx_rs_paid_at ON nb_settlements (paid_at);



-- -----------------------------------------------------
-- Table `nb_item_settlements` 子订单结算表
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_item_settlements`;

CREATE TABLE IF NOT EXISTS `nb_item_settlements` (
  `id`                      BIGINT        NOT NULL  AUTO_INCREMENT COMMENT '自增主键',
  `order_id`                BIGINT        NOT NULL  COMMENT '订单id',
...
CREATE INDEX idx_ris_order_id ON nb_item_settlements (order_id);
CREATE INDEX idx_ris_settled_at ON nb_item_settlements (settled_at);
CREATE INDEX idx_ris_paid_at ON nb_item_settlements (paid_at);


-- -----------------------------------------------------
-- Table `nb_deposit_fees` 商家保证金和技术服务费
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_deposit_fees`;

CREATE TABLE IF NOT EXISTS `nb_deposit_fees` (
  `id`              BIGINT      NOT NULL  AUTO_INCREMENT COMMENT '自增主键',
  `seller_id`       BIGINT      NOT NULL  COMMENT '商家id',
...
CREATE INDEX idx_rsf_seller_id ON `nb_deposit_fees` (seller_id);
CREATE INDEX idx_rsf_synced_at ON `nb_deposit_fees` (synced_at);


-- -----------------------------------------------------
-- Table `nb_service_fees`  平台技术服务费
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_deposit_account`;

CREATE TABLE IF NOT EXISTS `nb_deposit_account` (
  `id`            BIGINT      NOT NULL  AUTO_INCREMENT COMMENT '自增主键',
  `seller_id`     BIGINT      NOT NULL  COMMENT '商家id',
...
-- -----------------------------------------------------
-- Table `nb_daily_settlements`   日订单汇总
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_daily_settlements`;

CREATE TABLE IF NOT EXISTS `nb_daily_settlements` (
  `id`                      BIGINT    NOT NULL  AUTO_INCREMENT COMMENT '自增主键',
...
CREATE INDEX idx_rds_confirmed_at ON nb_daily_settlements (confirmed_at);


-- -----------------------------------------------------
-- Table `nb_seller_settlements`    商户订单日汇总
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_seller_settlements`;

CREATE TABLE IF NOT EXISTS `nb_seller_settlements` (
  `id`                      BIGINT      NOT NULL  AUTO_INCREMENT COMMENT '自增主键',
  `seller_id`               BIGINT      NOT NULL  COMMENT '商家id',
  `seller_name`             VARCHAR(64) NULL      COMMENT '商家名称',
  `outer_code`              VARCHAR(32) NULL      COMMENT '商家8码',
...
CREATE INDEX idx_rss_confirmed_at ON nb_seller_settlements (confirmed_at);
CREATE INDEX idx_rss_synced_at ON nb_seller_settlements (synced_at);
CREATE INDEX idx_rss_seller_id ON nb_seller_settlements (seller_id);



-- -----------------------------------------------------
-- Table `nb_alipay_cash`     支付宝日汇总
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_alipay_cash`;

CREATE TABLE IF NOT EXISTS `nb_alipay_cash` (
  `id`                      BIGINT    NOT NULL  AUTO_INCREMENT COMMENT '自增主键',
...
CREATE INDEX idx_rac_summed_at ON nb_alipay_cash (summed_at);


-- -----------------------------------------------------
-- Table `nb_seller_alipay_cash`  商户支付宝日汇总
-- -----------------------------------------------------

DROP TABLE IF EXISTS `nb_seller_alipay_cash`;

CREATE TABLE IF NOT EXISTS `nb_seller_alipay_cash` (
  `id`                      BIGINT      NOT NULL    AUTO_INCREMENT COMMENT '自增主键',
  `seller_id`               BIGINT      NOT NULL    COMMENT '商家id',
  `seller_name`             VARCHAR(64) NULL        COMMENT '商家名称',
  `outer_code`              VARCHAR(32) NULL        COMMENT '商家8码',
...
CREATE INDEX idx_rsac_summed_at ON nb_seller_alipay_cash (summed_at);
CREATE INDEX idx_rsac_union ON nb_seller_alipay_cash (summed_at, seller_id);


-- -----------------------------------------------------
-- Table `nb_settle_jobs`  对账任务
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_settle_jobs`;

CREATE TABLE IF NOT EXISTS `nb_settle_jobs` (
  `id`            BIGINT   NOT NULL   AUTO_INCREMENT COMMENT '自增主键',
...
CREATE INDEX idx_rsj_done_at ON nb_settle_jobs (done_at);
CREATE INDEX idx_rsj_traded_at ON nb_settle_jobs (traded_at);
CREATE INDEX idx_rsj_union ON nb_settle_jobs (done_at, `type`);


-- -----------------------------------------------------
-- Table `nb_business_rates`  费率
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_business_rates`;

CREATE TABLE IF NOT EXISTS `nb_business_rates` (
  `id`            BIGINT        NOT NULL   AUTO_INCREMENT COMMENT '自增主键',
...
-- -----------------------------------------------------
-- Table `nb_alipay_trans`  支付宝对账查询记录
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_alipay_trans_load`;

CREATE TABLE IF NOT EXISTS `nb_alipay_trans_load` (
  `id`                          BIGINT        NOT NULL   AUTO_INCREMENT COMMENT '自增主键',
...
-- -----------------------------------------------------
-- Table `nb_alipay_trans`  支付宝交易对账数据
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_alipay_trans`;

CREATE TABLE IF NOT EXISTS `nb_alipay_trans` (
  `id`                          BIGINT        NOT NULL   AUTO_INCREMENT COMMENT '自增主键',
...
CREATE INDEX idx_rat_trans_no ON nb_alipay_trans (trade_no);
CREATE INDEX idx_rat_merchant_no ON nb_alipay_trans (merchant_out_order_no);



-- -----------------------------------------------------
-- Table `nb_abnormal_trans`  挂账
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_abnormal_trans`;

CREATE TABLE IF NOT EXISTS `nb_abnormal_trans` (
  `id`                BIGINT        NOT NULL    AUTO_INCREMENT COMMENT '自增主键',
...
CREATE UNIQUE INDEX `idx_abnormal_settlement_id_uniq` ON nb_abnormal_trans (`settlement_id` ASC);

-- -----------------------------------------------------
-- Table `nb_deposit_fee_cash`  支付宝押金提现单
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nb_deposit_fee_cash`;

CREATE TABLE IF NOT EXISTS `nb_deposit_fee_cash` (
  `id`          BIGINT      NOT NULL    AUTO_INCREMENT COMMENT '自增主键',
...
-- -----------------------------------------------------
-- Table `nb_order_alipay_cash`  订单提现明细
-- -----------------------------------------------------

DROP TABLE IF EXISTS `nb_order_alipay_cash`;

CREATE TABLE IF NOT EXISTS `nb_order_alipay_cash` (
  `id`              BIGINT        NOT NULL    AUTO_INCREMENT COMMENT '自增主键',
  `order_id`        BIGINT        NOT NULL    COMMENT '订单id',
...
CREATE INDEX idx_roac_traded_at ON nb_order_alipay_cash (traded_at);
CREATE INDEX idx_roac_order_id ON nb_order_alipay_cash (order_id);
CREATE INDEX idx_roac_seller_id ON nb_order_alipay_cash (seller_id);
